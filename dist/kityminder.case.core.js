/*!
 * ====================================================
 * Kity Minder Core For Case - v2.1.13 - 2024-01-10
                    ...
                    if (!stoped) {
                        ...
                        if (cmd.isContentChanged()) {
                            this._firePharse(new MinderEvent("contentchange", {
                                datas: this.queryCommandValue("datas"),
                                reason: "execCommand: " + name
                            }));
                        }
    ...
    value: function(require, exports, module) {
        ...
        kity.extendClass(Minder, {
            ...
            updateConnect: function(node) {
                ...
                if (parent.isCollapsed() || node.hide) {
         ...
         */
            Text2Children: function(node, text) {
                ...
                function importChildren(node, children, type, curLevel, maxLevel) {
                    ...
                    if (type == null) {
                        //自动判断节点类型
                        if (maxLevel >= 4 && curLevel <= maxLevel - 4) {
                            // 层级大于等于4，除了后3个层级，其他都是目录类型
                            autoType = 1;
                        } else {
                            autoType = 2;
                        }
     ...
     */
         ...
         */
            ...
            getData: function(key) {
                if (key) {
                    if (this.data.hasOwnProperty(key)) return this.data[key]; else return "";
                }
                return this.data;
            },
         ...
         */
            ...
            calculatePath: function(parent) {
                if (!parent) return null; else if (parent.data.path) {
                    //父节点有路径
            ...
            getDescendantsLevelID: function() {
                ...
                d.forEach(function(node) {
                    ...
                    if (idLevelMap[level]) idLevelMap[level].push(node.data.id); else idLevelMap[level] = [ node.data.id ];
                });
                return idLevelMap;
            },
...
_p[32] = {
     ...
     */
         ...
         */
            getTheme: function(node) {
                return this._theme || this.getOption("defaultTheme") || "fresh-blue-compat";
            },
        ...
        Module.register("ClipboardModule", function() {
            ...
            function appendChildNode(parent, child) {
                ...
                var children = child.children.map(function(node) {
                    ...
                    newNode.data.parent_id = child.data.id;
                    return newNode;
                });
                /*
                    ...
                    if (_clipboardNodes.length) {
                        ...
                        for (var i = 0, ni; ni = _clipboardNodes[i]; i++) {
                            for (var j = 0, node; node = nodes[j]; j++) {
                                var newNi = ni.clone();
                                appendChildNode(node, newNi);
                                newNodesData.push(newNi.data);
                            }
                        }
    ...
    value: function(require, exports, module) {
        ...
        var TreeDragger = kity.createClass("TreeDragger", {
            ...
            dragEnd: function() {
                ...
                this._leaveDragMode();
                var datas = [];
                this._minder.getSelectedNodes().forEach(function(node) {
                    datas.push(node.getDataForChange());
                });
     ...
     */
        var AppendNextCommand = kity.createClass("AppendNextCommand", {
            ...
            execute: function(km, text) {
                ...
                if (parentType == typeMap.expect.id) {
                    //父节点是预期结果，无法新增子节点
                    return null;
                } else if (parentType == typeMap.module.id) {
                    //父节点是模块，子节点应是用例
                    data = {
                        text: text || "用例名称",
                        type: typeMap.case.id
                    };
                } else if (parentType == typeMap.case.id) {
                    //父节点是用例，子节点应是步骤
                    data = {
                        text: text || "操作步骤",
                        type: typeMap.step.id
                    };
                } else if (parentType == typeMap.step.id) {
                    //父节点是步骤，子节点应是预期
                    data = {
                        text: text || "预期结果",
            ...
            queryState: function(km) {
                ...
                var typeMap = km.getTypeMap();
                for (var i = 0; i < nodes.length; i++) {
                    if (nodes[i].getData("type") == typeMap.expect.id) return -1;
                }
                return 0;
            }
        });
     ...
     */
        var AppendPrevCommand = kity.createClass("AppendPrevCommand", {
            ...
            execute: function(km, text) {
                ...
                if (nodeType == typeMap.module.id || nodeType == typeMap.case.id) {
                    //当前节点是模块或用例，父节点应是模块
                    data = {
                        text: text || "模块名称",
                        type: typeMap.module.id
                    };
                } else if (nodeType == typeMap.step.id) {
                    //当前节点是步骤，父节点应是用例
                    data = {
                        text: text || "用例名称",
                        type: typeMap.case.id
                    };
                } else if (nodeType == typeMap.expect.id) {
                    //当前节点是预期，父节点应是步骤
                    data = {
                        text: text || "操作步骤",
                        type: typeMap.step.id
     ...
     */
        var AppendSameCommand = kity.createClass("AppendSameCommand", {
            ...
            execute: function(km, text) {
                ...
                if (!text) {
                    if (siblingType == typeMap.module.id) text = "模块名称"; else if (siblingType == typeMap.case.id) text = "用例名称"; else if (siblingType == typeMap.step.id) text = "操作步骤"; else if (siblingType == typeMap.expect.id) text = "预期结果";
                }
                var data = {
                    text: text,
                    type: siblingType || typeMap.module.id
                };
                var parent = sibling.parent;
                if (!parent) {
         ...
         */
            ...
            var NoteIcon = kity.createClass("NoteIcon", {
                ...
                constructor: function() {
                    ...
                    this.text = new kity.Text().setFontSize(11).setVerticalAlign("middle");
        ...
        Module.register("PriorityModule", function() {
            ...
            var PriorityIcon = kity.createClass("PriorityIcon", {
                ...
                create: function() {
                    ...
                    mask = new kity.Path().setPathData(MASK_PATH).setOpacity(.8).setTranslate(-3, -2);
                    number = new kity.Text().setX(this.width / 2 - 1).setY(this.height / 2).setTextAnchor("middle").setVerticalAlign("middle").setFontItalic(true).setFontSize(12).fill("white");
        ...
        Module.register("Type", function() {
            ...
            var getTypeByID = function(typeId) {
                for (var key in types) {
                    if (types[key].id == typeId) return types[key];
                }
                return {};
            };
            kity.extendClass(Minder, {
         ...
         */
            var TypeOverlay = kity.createClass("TypeOverlay", {
                ...
                setValue: function(type) {
                    ...
                    text.setX(paddingX).setY(paddingY * 2 - 3).fill("#4d4100");
                    //字体颜色
                    rect = this.rect;
         ...
         */
            var TypeRenderer = kity.createClass("TypeRenderer", {
                ...
                update: function(container, node, box) {
                    ...
                    if (container.getShapes() && container.getShapes().length > 0) container.removeShape(0);
                    //先删除再新增，避免发生重影
                    container.addShape(overlay);
                    overlay.setVisible(true);
         ...
         */
            var AlignLeftCommand = kity.createClass("AlignLeftCommand", {
                ...
                execute: function(km, focusNode) {
                    ...
                    var viewport = km.getPaper().getViewPort();
                    var offset = focusNode.getRenderContainer().getRenderBox("view");
                    var dx = viewBox.x - offset.x - offset.width / 2, dy = viewport.center.y - offset.y;
                    var dragger = km._viewDragger;
                    var duration = km.getOption("viewAnimationDuration");
                    dragger.move(new kity.Point(dx, dy), duration);
                    this.setContentChanged(false);
